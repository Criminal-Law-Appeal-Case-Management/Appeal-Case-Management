...
const getCategoryColor = (category) => {
  const colors = {
    ...
    evidence: "bg-sky-50 text-sky-700 border-sky-200",
  ...
  const highlightMatch = (text, query) => {
    ...
    return parts.map((part, i) => 
      part.toLowerCase() === query.toLowerCase() 
        ? <mark key={i} className="bg-sky-200 px-0.5">{part}</mark> 
        : part
    );
  };
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <label htmlFor="files" className="cursor-pointer">
                <FileUp className={`w-10 h-10 mx-auto mb-3 ${isDragging ? 'text-sky-500' : 'text-slate-400'}`} />
                <p className="text-sm font-medium text-slate-700">
                  {isDragging ? 'Drop files here' : 'Drag & drop files here'}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  or <span className="text-sky-600 hover:underline">browse</span> to select
                </p>
