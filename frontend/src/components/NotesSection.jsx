        ...
        <div className="grid gap-4" data-testid="notes-list">
          {sortedNotes.map((note) => {
            ...
            return (
              <Card
                key={note.note_id}
                className={`hover:shadow-md transition-shadow group ${pinned ? "border-sky-300 bg-sky-50/30" : ""}`}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        {pinned && <Pin className="w-4 h-4 text-sky-500" data-testid={`note-pinned-icon-${note.note_id}`} />}
