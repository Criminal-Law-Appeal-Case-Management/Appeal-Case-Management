...
const Dashboard = ({ user }) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Sidebar */}
      <aside className={`fixed left-0 top-8 bottom-0 w-72 bg-card border-r border-border p-6 z-40 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        {/* Logo */}
        ...
        <nav className="space-y-2">
          <Link to="/" className="sidebar-item" data-testid="nav-home">
          ...
          <Link to="/forms" className="sidebar-item" data-testid="nav-forms">
            <FileCheck className="w-5 h-5" />
            <span className="font-medium">Forms & Templates</span>
          </Link>
          <Link to="/lawyers" className="sidebar-item" data-testid="nav-lawyers">
            <Users className="w-5 h-5" />
            <span className="font-medium">Find Lawyers</span>
          </Link>
        ...
        <div className="p-6 lg:p-8 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-muted/50 border border-border rounded-xl p-4">
              <p className="text-sm text-muted-foreground leading-relaxed">
                Designed for analysing appeal issues across <strong className="text-foreground">all criminal offences</strong> in Australian courts, 
                including questions relating to mens rea, procedural error, fresh evidence, and sentencing.
              </p>
            </div>
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
              <p className="text-sm text-amber-800 dark:text-amber-200 leading-relaxed">
                <strong>Privacy:</strong> Ensure you have lawful authority to upload any court documents or personal information.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2" style={{ fontFamily: 'Crimson Pro, serif' }}>
              ...
              {filteredCases.map((caseItem) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <h3 
                        className="text-lg font-semibold text-foreground group-hover:text-amber-600 transition-colors truncate"
                        style={{ fontFamily: 'Crimson Pro, serif' }}
                      >
                        {caseItem.title}
                      </h3>
                      <p className="text-sm text-muted-foreground mt-1">{caseItem.defendant_name}</p>
                    </div>
                    <DropdownMenu>
                  ...
                  {caseItem.case_number && (
                    <p className="text-xs text-muted-foreground font-mono mb-4">{caseItem.case_number}</p>
                  )}
